class Course < ApplicationRecord
  ...
  def self.export_khaksar
    ...
    rows = ApplicationRecord.connection.exec_query("SELECT exams + exercises + session_durations + teacher_view + student_view + resources AS s, serial, fullname FROM public.course_percentiles join public.courses on  public.course_percentiles.course_id = public.courses.mid join public.course_teachers on  public.course_percentiles.course_id = public.course_teachers.course_id ORDER BY s desc LIMIT 50").rows
